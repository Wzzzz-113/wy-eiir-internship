...
def xyxyxyxy2xywhr(x):
    ...
    to [cx, cy, w, h, rotation] format, with angle normalized to [-pi/4, pi/4].
    """
    ...
    rboxes = []
    for pts in points:
        (cx, cy), (w, h), angle = cv2.minAreaRect(pts)  # angle in degrees, [-90, 0)
        angle = angle / 180 * np.pi  # to radians

        # ✅ 关键步骤：把角度限制到 [-pi/4, pi/4]
